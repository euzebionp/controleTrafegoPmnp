...
def travels_page():
    ...
    with tab1:
        ...
        with st.form("travel_form"):
                ...
                if st.session_state.get("last_calc_key") != key:
                    with st.spinner("Calculando distância..."):
                        ...
                        if dist:
                            ...
                            st.warning(
                                "Não foi possível calcular a distância automaticamente."
                            )

            # Distance
            default_dist = (
                st.session_state.get("last_calc_dist", 0.0)
                if (
                    origem
                    and destino
                    and st.session_state.get("last_calc_key") == f"{origem}-{destino}"
                )
                else 0.0
            )
            ...
            if drivers_df.empty:
                st.warning(
                    "⚠️ Nenhum motorista cadastrado. Cadastre um motorista primeiro."
                )
                driver_options = []
            else:
                driver_options = {
            ...
            selected_driver = st.selectbox(
                ...
                options=(
                    ...
                    if driver_options
                    else ["Nenhum motorista disponível"]
                ),
            ...
            if vehicles_df.empty:
                ...
                vehicle_options = {
                    f"{row['placa']} - {row['modelo']} ({row['ano']})": row["id"]
            ...
            selected_vehicle = st.selectbox(
                ...
                options=(
                    ...
                    if vehicle_options
                    else ["Nenhum veículo disponível"]
                ),
                disabled=len(vehicle_options) == 0,
            )

            # Current Mileage (Optional/Alternative to Distance)
            km_atual_input = st.number_input(
                "Km Atual (Odômetro Final)",
                min_value=0.0,
                step=1.0,
                help="Preencha se souber o odômetro final. Isso atualizará o Km atual do veículo.",
            )

            submit_button = st.form_submit_button(
                "🚗 Cadastrar Viagem", use_container_width=True
            )

            if submit_button:
                if not driver_options or not vehicle_options:
                    st.error(
                        "Cadastre motoristas e veículos antes de registrar uma viagem."
                    )
    ...
    with tab2:
        ...
        with col_filter1:
            ...
            date_range = st.date_input(
                "Filtrar por Data", value=(last_30, today), format="DD/MM/YYYY"
            )
        ...
        if not travels_df.empty:
            ...
            if sort_option == "Cidade Destino":
                ...
                travels_df = travels_df.sort_values(
                    by=["data", "hora_saida"], ascending=[False, False]
                )
        ...
        if travels_df.empty:
            ...
            with col3:
                total_km = (
                    travels_df["distancia"].sum()
                    if "distancia" in travels_df.columns
                    else 0
                )
            ...
            for index, row in travels_df.iterrows():
                ...
                with st.expander(title):
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**📏 Distância:** {row['distancia']:.0f} km"
                            if pd.notna(row["distancia"])
                            else "**📏 Distância:** 0 km"
                        )
                        ...
                        st.write(
                            f"**🚙 Veículo:** {row['veiculo_placa']} - {row['veiculo_modelo']}"
                        )
                    ...
                    if st.session_state.get(f'editing_travel_{row["id"]}', False):
                        ...
                        with st.form(f"edit_form_{row['id']}"):
                            ...
                            with col1:
                                try:
                                    date_obj = datetime.strptime(
                                        travel_data["data"], "%Y-%m-%d"
                                    ).date()
                                except:
                                    date_obj = datetime.now().date()
                                edit_data = st.date_input(
                                    "Data da Viagem", value=date_obj
                                )

                            with col2:
                                try:
                                    time_obj = datetime.strptime(
                                        travel_data["hora_saida"], "%H:%M"
                                    ).time()
                                except:
                                    time_obj = time(8, 0)
                                edit_hora_saida = st.time_input(
                                    "Hora de Saída", value=time_obj
                                )

                            # Origin and Destination
                            edit_origem = st.text_input(
                                "Origem", value=travel_data.get("origem", "")
                            )
                            edit_destino = st.text_input(
                                "Destino", value=travel_data["destino"]
                            )
                            ...
                            try:
                                ...
                                dist_value = 0.0
                            edit_distancia = st.number_input(
                                "Distância Percorrida (Km)",
                                min_value=0.0,
                                step=1.0,
                                value=dist_value,
                            ...
                            vehicles_df = db_handler.get_vehicles()
                            vehicle_options = {
                                f"{row['placa']} - {row['modelo']} ({row['ano']})": row[
                                    "id"
                                ]
                                for _, row in vehicles_df.iterrows()
                            }
                            ...
                            try:
                                ...
                                val_km_atual = 0.0
                            edit_km_atual = st.number_input(
                                "Km Atual (Odômetro Final)",
                                min_value=0.0,
                                step=1.0,
                                value=val_km_atual,
                                key=f"edit_km_{row['id']}",
                            )
