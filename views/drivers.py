...
def generate_drivers_pdf(drivers_df):
    ...
    elements.append(
        Paragraph(f"Total de Motoristas: {total_drivers}", styles["Normal"])
    )
    ...
    for index, row in drivers_df.iterrows():
        ...
        data.append([row["nome"], row["cpf"], row["cnh"], formatted_date, status_text])
...
def drivers_page():
        ...
        with st.form("driver_form"):
            ...
            if submit_button:
                if nome and cpf and cnh and validade_cnh:
                    success, message = db_handler.add_driver(
                        nome, cpf, cnh, str(validade_cnh)
                    )
                    if success:
                        st.success(message)
                        st.rerun()
    ...
    with tab2:
        ...
        if drivers_df.empty:
            ...
            if expired_drivers:
                st.error(
                    f"⚠️ **ALERTA:** {len(expired_drivers)} motorista(s) com CNH vencida: {', '.join(expired_drivers)}"
                )
            ...
            with col_print:
                if st.button(
                    "🖨️ Imprimir Lista de Motoristas", use_container_width=True
                ):
            ...
            for index, row in drivers_df.iterrows():
                # Get CNH status
                status_text, status_color, icon = utils.get_cnh_status(
                    row["validade_cnh"]
                )
                ...
                with st.expander(title):
                    ...
                    with col1:
                        ...
                        if status_text == "VENCIDA":
                            st.markdown(
                                f"**Validade CNH:** :red[{formatted_date}] 🔴 **{status_text}**"
                            )
                        elif (
                            "Vence em" in status_text
                            and utils.days_until_expiration(row["validade_cnh"]) <= 30
                        ):
                            st.markdown(
                                f"**Validade CNH:** :orange[{formatted_date}] ⚠️ **{status_text}**"
                            )
                        elif (
                            "Vence em" in status_text
                            and utils.days_until_expiration(row["validade_cnh"]) <= 90
                        ):
                            st.markdown(
                                f"**Validade CNH:** :yellow[{formatted_date}] ⚡ **{status_text}**"
                            )
                        else:
                            st.markdown(
                                f"**Validade CNH:** :green[{formatted_date}] ✅ **{status_text}**"
                            )
                        ...
                        with st.form(f"edit_form_{row['id']}"):
                            ...
                            try:
                                date_obj = datetime.strptime(
                                    row["validade_cnh"], "%Y-%m-%d"
                                ).date()
                            except:
                                date_obj = datetime.now().date()

                            edit_validade = st.date_input(
                                "Validade da CNH", value=date_obj
                            )
